...
data class V2rayConfig(
    ...
    data class OutboundBean(
        ...
        fun getServerAddress(): String? {
            if (protocol.equals(protocols.VMESS.name, true)
                || protocol.equals(protocols.VLESS.name, true)
            ) {
                return settings?.vnext?.get(0)?.address
            } else if (protocol.equals(protocols.SHADOWSOCKS.name, true)
                || protocol.equals(protocols.SOCKS.name, true)
                || protocol.equals(protocols.TROJAN.name, true)
            ) {
    ...
    fun getProxyOutbound(): OutboundBean? {
        outbounds.forEach { outbound ->
            if (outbound.protocol.equals(protocols.VMESS.name, true) ||
                outbound.protocol.equals(protocols.VLESS.name, true) ||
                outbound.protocol.equals(protocols.SHADOWSOCKS.name, true) ||
                outbound.protocol.equals(protocols.SOCKS.name, true) ||
                outbound.protocol.equals(protocols.TROJAN.name, true)
            ) {
                return outbound
            }
        }
