...
class V2RayVpnService : VpnService(), ServiceControl {
    ...
    private fun setup(parameters: String) {
        ...
        val builder = Builder()
        val enableLocalDns = settingsStorage?.decodeBool(Constants.PREF_LOCAL_DNS_ENABLED) ?: false
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP &&
            settingsStorage?.decodeBool(Constants.PREF_PER_APP_PROXY) == true
        ) {
            val apps = settingsStorage?.decodeStringSet(Constants.PREF_PER_APP_PROXY_SET)
            val bypassApps = settingsStorage?.decodeBool(Constants.PREF_BYPASS_APPS) ?: false
            apps?.forEach {
                try {
                    if (bypassApps)
