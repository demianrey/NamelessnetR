...
import namelessnet.org.Constants.ANG_CONFIG
import namelessnet.org.Constants.HTTPS_PROTOCOL
import namelessnet.org.Constants.HTTP_PROTOCOL
import namelessnet.org.R
import namelessnet.org.core.*
import namelessnet.org.core.V2rayConfig.Companion.DEFAULT_SECURITY
import namelessnet.org.core.V2rayConfig.Companion.TLS
...
//                Constants.PREF_LOCAL_DNS_PORT,
//                Constants.PREF_SOCKS_PORT,
//                Constants.PREF_HTTP_PORT,
//                Constants.PREF_LOGLEVEL,
...
//                Constants.PREF_PREFER_IPV6,
        ...
        settingsStorage?.encode(
            ...
            sharedPreferences.getBoolean(Constants.PREF_SNIFFING_ENABLED, true)
        )
        settingsStorage?.encode(
            Constants.PREF_PER_APP_PROXY_SET,
            sharedPreferences.getStringSet(Constants.PREF_PER_APP_PROXY_SET, setOf())
        )
    }
...
//                    }
    ...
    private fun importConfig(
        ...
        try {
            ...
            if (str.startsWith(protocols.VMESS.protocolScheme)) {
                ...
                if (!tryParseNewVmess(str, config, allowInsecure)) {
                    if (str.indexOf("?") > 0) {
                        ...
                        var result = str.replace(protocols.VMESS.protocolScheme, "")
            ...
            } else if (str.startsWith(protocols.SHADOWSOCKS.protocolScheme)) {
                var result = str.replace(protocols.SHADOWSOCKS.protocolScheme, "")
                val indexSplit = result.indexOf("#")
                config = ServerConfig.create(protocols.SHADOWSOCKS)
                if (indexSplit > 0) {
                    try {
                        config.remarks =
            ...
            } else if (str.startsWith(protocols.SOCKS.protocolScheme)) {
                var result = str.replace(protocols.SOCKS.protocolScheme, "")
                val indexSplit = result.indexOf("#")
                config = ServerConfig.create(protocols.SOCKS)
                if (indexSplit > 0) {
                    try {
                        config.remarks =
                            Utils.urlDecode(result.substring(indexSplit + 1, result.length))
            ...
            } else if (str.startsWith(protocols.TROJAN.protocolScheme)) {
                val uri = URI(str)
                config = ServerConfig.create(protocols.TROJAN)
            ...
            } else if (str.startsWith(protocols.VLESS.protocolScheme)) {
                val uri = URI(str)
                val queryParam = uri.rawQuery.split("&")
                    .associate { it.split("=").let { (k, v) -> k to Utils.urlDecode(v) } }
                config = ServerConfig.create(protocols.VLESS)
                val streamSetting = config.outboundBean?.streamSettings ?: return -1
                config.remarks = uri.fragment ?: ""
                config.outboundBean?.settings?.vnext?.get(0)?.let { vnext ->
    ...
    private fun tryResolveVmess4Kitsunebi(server: String, config: ServerConfig): Boolean {
        ...
        var result = server.replace(protocols.VMESS.protocolScheme, "")
        val indexSplit = result.indexOf("?")
        if (indexSplit > 0) {
            result = result.substring(0, indexSplit)
        }
