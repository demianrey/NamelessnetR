...
import namelessnet.org.core.V2rayConfig.Companion.DEFAULT_NETWORK
import namelessnet.org.core.V2rayConfig.Companion.HTTP
...
object V2rayConfigUtil {
    ...
    fun getV2rayConfig(context: Context, guid: String): Result {
        try {
            ...
            if (config.configType == protocols.PRIVATE) {
                ...
                val PRIVATEConfig = if (raw.isNullOrBlank()) {
                    config.fullConfig?.toPrettyPrinting() ?: return Result(false, "")
                } else {
                    raw
                }
                Log.d(ANG_PACKAGE, PRIVATEConfig)
                return Result(true, PRIVATEConfig)
            }
            val outbound = config.getProxyOutbound() ?: return Result(false, "")
            val result = getV2rayNonPRIVATEConfig(context, outbound)
     ...
     * 生成v2ray的客户端配置文件
     */
    private fun getV2rayNonPRIVATEConfig(
        context: Context,
        ...
        //v2rayConfig.log.loglevel = settingsStorage?.decodeString(Constants.PREF_LOGLEVEL) ?: "warning"
        ...
        if (settingsStorage?.decodeBool(Constants.PREF_LOCAL_DNS_ENABLED) == true) {
            PRIVATELocalDns(v2rayConfig)
        }
        if (settingsStorage?.decodeBool(Constants.PREF_SPEED_ENABLED) != true) {
    ...
    private fun inbounds(v2rayConfig: V2rayConfig): Boolean {
        try {
            //val socksPort = Utils.parseInt(settingsStorage?.decodeString(Constants.PREF_SOCKS_PORT) ?: Constants.PORT_SOCKS)
            //val httpPort = Utils.parseInt(settingsStorage?.decodeString(Constants.PREF_HTTP_PORT) ?: Constants.PORT_HTTP)
            ...
            val fakedns = settingsStorage?.decodeBool(Constants.PREF_FAKE_DNS_ENABLED)
                ?: false
            val sniffAllTlsAndHttp =
                settingsStorage?.decodeBool(Constants.PREF_SNIFFING_ENABLED, true)
                    ?: true
...
//            }
    ...
    private fun fakedns(v2rayConfig: V2rayConfig) {
        if (settingsStorage?.decodeBool(Constants.PREF_FAKE_DNS_ENABLED) == true) {
    ...
    private fun routing(v2rayConfig: V2rayConfig): Boolean {
        try {
            routingUserRule(
                settingsStorage?.decodeString(Constants.PREF_V2RAY_ROUTING_AGENT)
                    ?: "", Constants.TAG_AGENT, v2rayConfig
            )
            routingUserRule(
                settingsStorage?.decodeString(Constants.PREF_V2RAY_ROUTING_DIRECT)
                    ?: "", Constants.TAG_DIRECT, v2rayConfig
            )
            routingUserRule(
                settingsStorage?.decodeString(Constants.PREF_V2RAY_ROUTING_BLOCKED)
                    ?: "", Constants.TAG_BLOCKED, v2rayConfig
            )

            v2rayConfig.routing.domainStrategy =
                settingsStorage?.decodeString(Constants.PREF_ROUTING_DOMAIN_STRATEGY)
                    ?: "IPIfNonMatch"
            val routingMode = settingsStorage?.decodeString(Constants.PREF_ROUTING_MODE) ?: "0"

            // Hardcode googleapis.cn
            val googleapisRoute = V2rayConfig.RoutingBean.RulesBean(
                type = "field",
                outboundTag = Constants.TAG_AGENT,
                domain = arrayListOf("domain:googleapis.cn")
            )

            when (routingMode) {
                "1" -> {
                    routingGeo("ip", "private", Constants.TAG_DIRECT, v2rayConfig)
                }
                "2" -> {
                    routingGeo("", "cn", Constants.TAG_DIRECT, v2rayConfig)
...
//                                    || it.startsWith("full:"))
     ...
     * PRIVATE Dns
     */
    private fun PRIVATELocalDns(v2rayConfig: V2rayConfig): Boolean {
        try {
            if (settingsStorage?.decodeBool(Constants.PREF_FAKE_DNS_ENABLED) == true) {
            ...
            if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
                ...
                //val localDnsPort = Utils.parseInt(settingsStorage?.decodeString(Constants.PREF_LOCAL_DNS_PORT) ?: Constants.PORT_LOCAL_DNS)
