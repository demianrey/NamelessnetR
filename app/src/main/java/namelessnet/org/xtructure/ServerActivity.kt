...
import namelessnet.org.core.V2rayConfig.Companion.DEFAULT_PORT
import namelessnet.org.core.V2rayConfig.Companion.XTLS
import namelessnet.org.core.protocols
import namelessnet.org.xtreme.BaseActivity
import namelessnet.org.xtreme.MmkvManager
import namelessnet.org.xtreme.MmkvManager.ID_MAIN
import namelessnet.org.xtreme.MmkvManager.KEY_SELECTED_SERVER
...
class ServerActivity : BaseActivity() {
    ...
    private val creatprotocols by lazy {
        protocols.fromInt(intent.getIntExtra("creatprotocols", protocols.VMESS.value))
            ?: protocols.VMESS
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (config?.configType ?: creatprotocols) {
            protocols.VMESS -> setContentView(R.layout.activity_server_vmess)
            protocols.PRIVATE -> return
            protocols.SHADOWSOCKS -> setContentView(R.layout.activity_server_shadowsocks)
            protocols.SOCKS -> setContentView(R.layout.activity_server_socks)
            protocols.VLESS -> setContentView(R.layout.activity_server_vless)
            protocols.TROJAN -> setContentView(R.layout.activity_server_trojan)
        }
    ...
    private fun bindingServer(config: ServerConfig): Boolean {
        ...
        if (config.configType == protocols.SOCKS) {
            et_security.text =
        ...
        val securityEncryptions =
            if (config.configType == protocols.SHADOWSOCKS) shadowsocksSecuritys else securitys
        val security =
    ...
    private fun saveServer(): Boolean {
        ...
        val config = MmkvManager.decodeServerConfig(editGuid) ?: ServerConfig.create(creatprotocols)
        if (config.configType != protocols.SOCKS && TextUtils.isEmpty(et_id.text.toString())) {
    ...
    private fun saveVnext(
        ...
        if (config.configType == protocols.VMESS) {
...
//                vnext.users[0].flow = flows[sp_flow.selectedItemPosition].ifBlank { V2rayConfig.DEFAULT_FLOW }
    ...
    private fun saveServers(
        ...
        } else if (config.configType == protocols.TROJAN) {
            server.password = et_id.text.toString().trim()
        }
    }

    private fun saveStreamSettings(
        streamSetting: V2rayConfig.OutboundBean.StreamSettingsBean,
        config: ServerConfig
    ) {
        val network =
        ...
        val requestHost =
            if (et_request_host != null) et_request_host.text.toString().trim() else ""
        ...
        val allowInsecure =
            if (sp_allow_insecure == null || allowinsecures[sp_allow_insecure.selectedItemPosition].isBlank()) {
        ...
        val defaultTls = if (config.configType == protocols.TROJAN) V2rayConfig.TLS else ""
